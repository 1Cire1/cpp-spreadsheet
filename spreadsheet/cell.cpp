...
void Cell::Set(std::string text) {
    ...
    if (!temp_ref_cells.empty()) {
        ...
        while (!to_enter_collection.empty()) {
            ...
            const Cell* extant = to_enter_collection.back();    
            to_enter_collection.pop_back();
            enter_collection.insert(extant);
            
            if (ref_collection.find(extant) == ref_collection.end()) {
                
                for (const Cell* dependent : extant->current_cells_) {
    ...
    for (Cell* refrenced : referenced_cells_) {
        refrenced->current_cells_.erase(this);
    }
    ...
    for (const auto& position : impl_->GetReferencedCells()) {
        ...
        refrenced->current_cells_.insert(this);
    }
...
void Cell::InvalidateAllCache(bool flag = false) {
    if (impl_->HasCache() || flag) {
        ...
        for (Cell* dependent : current_cells_) {
