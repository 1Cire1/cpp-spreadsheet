...
void Sheet::SetCell(Position pos, std::string text) { 
    ...
    if (pos.IsValid()) { 
        ...
        throw InvalidPositionException("Invalid_Cell_Position.SetCell()");
    }    
}
...
const CellInterface* Sheet::GetCell(Position pos) const {
    ...
    if (pos.IsValid()) {
        ...
        throw InvalidPositionException("Invalid_Cell_Position.GetCell()");
    }
}
...
void Sheet::ClearCell(Position pos) {
    ...
    if (pos.IsValid()) {
        ...
        throw InvalidPositionException("Invalid_Cell_Position.ClearCell()");
    } 
}
    ...
    for (int row = 0; row < GetPrintableSize().rows; ++row) {     
        for (int col = 0; col < GetPrintableSize().cols; ++col) {
            ...
            if (col < int(std::size(cells_[row]))) {                  
                if (cells_[row][col]) {std::visit([&output](const auto& obj) {
                    output << obj;
                }, 
